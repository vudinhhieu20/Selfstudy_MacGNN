...
class NeighborAggregation(nn.Module):
    ...
    def forward(self, query, key):
        ...
        score = self.get_score(
            torch.bmm(trans_Q, torch.transpose(trans_K, 1, 2)) / (self.trans_d)
        )
...
class MacGNN(nn.Module):
    ...
    def __init__(
        ...
        self.u_gruop_slice = torch.arange(self.u_group_num, requires_grad=False).to(
            device
        )
        self.i_gruop_slice = torch.arange(self.i_group_num, requires_grad=False).to(
            device
        )
    ...
    def forward(self, x):
        ...
        user_2ord_neighbor = x[
            :, self.i_group_num + 1 : self.i_group_num + self.u_group_num + 1
        ]
        user_recent = x[
            :,
            self.i_group_num
        ...
        item_embedding = self.item_embed(
            x[:, self.i_group_num + self.u_group_num + self.recent_len + 1]
        )
        item_1ord_neighbor = x[
            :,
            self.i_group_num
            + self.u_group_num
            + self.recent_len
        ...
        item_2ord_neighbor = x[
            :,
            self.i_group_num
            + 2 * self.u_group_num
            + self.recent_len
            + 2 : 2 * self.i_group_num
            + 2 * self.u_group_num
            + self.recent_len
            + 2,
        ]
        item_recent = x[
            :, 2 * self.i_group_num + 2 * self.u_group_num + self.recent_len + 2 :
        ]
        ...
        user_1ord_weight = self.macro_weight_func(
            torch.log(user_1ord_neighbor.float() + 1) / self.tau
        ).unsqueeze(-1)
        user_2ord_weight = self.macro_weight_func(
            torch.log(user_2ord_neighbor.float() + 1) / self.tau
        ).unsqueeze(-1)
        item_1ord_weight = self.macro_weight_func(
            torch.log(item_1ord_neighbor.float() + 1) / self.tau
        ).unsqueeze(-1)
        item_2ord_weight = self.macro_weight_func(
            torch.log(item_2ord_neighbor.float() + 1) / self.tau
        ).unsqueeze(-1)
        ...
        u_1ord_trans_emb = self.i_shared_aggregator(
            user_1ord_embedding, item_embedding.unsqueeze(1)
        )
        u_2ord_trans_emb = self.u_shared_aggregator(
            user_2ord_embedding, user_embedding.unsqueeze(1)
        )
        i_1ord_trans_emb = self.u_shared_aggregator(
            item_1ord_embedding, user_embedding.unsqueeze(1)
        )
        i_2ord_trans_emb = self.i_shared_aggregator(
            item_2ord_embedding, item_embedding.unsqueeze(1)
        )
        user_recent_trans_emb = self.i_shared_aggregator(
            user_recent_embedding, item_embedding.unsqueeze(1)
        )
        item_recent_trans_emb = self.u_shared_aggregator(
            item_recent_embedding, user_embedding.unsqueeze(1)
        )
