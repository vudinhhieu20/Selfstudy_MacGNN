...
if args.dataset_name == "ml-10m":
    with open(f"../data/{dataset_name}.pkl", "rb") as f:
        ...
        user_count, item_count, cate_count, u_cluster_num, i_cluster_num = pickle.load(
            f
        )
...
train_data = DatasetBuilder(
    data=train_set, user_count=user_count, item_count=item_count
)
    ...
    for epo in range(epochs):
        for i, (fields, target) in enumerate(tk0):
            ...
            if now_iter % test_iter == 0:
                # testing
                auc, log_losses, _ = evaluation(
                    model, test_data_loader, device, use_gauc=False
                )
                if not early_stopper.is_continuable(model, auc, log_losses):
                    print(
                        f"validation: best auc: {early_stopper.best_auc}, best logloss: {early_stopper.best_logloss}"
                    )
                    break_flag = True
                    break
...
def evaluation(model, data_loader, device, use_gauc=False):
    ...
    return (
        metrics.roc_auc_score(targets, predicts),
        metrics.log_loss(targets, predicts),
        gauc,
    )
...
for now_run in range(args.runs):
    ...
    early_stopper = EarlyStopper(
        num_trials=trials, save_path=f"{model_name}_{dataset_name}.pt"
    )
...
auc_mean, auc_std = np.mean(np.array(auc_runs), axis=0), np.std(
    np.array(auc_runs), axis=0
)
logloss_mean, logloss_std = np.mean(np.array(logloss_runs), axis=0), np.std(
    np.array(logloss_runs), axis=0
)
gauc_mean, gauc_std = np.mean(np.array(gauc_runs), axis=0), np.std(
    np.array(gauc_runs), axis=0
)
